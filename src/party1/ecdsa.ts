...
export class EcdsaParty1 extends Party1 {
  ...
  public async getMasterKey(masterKeyId: string): Promise<EcdsaParty1Share> {
    if (!this.getRocksDb()) {
      throw new Error(
        "RocksDB not initialized. The DB path should be passed in the constructor."
      );
    }
    ...
    return JSON.parse(
      await this.getRocksDb().get(searchString, { asBuffer: false })
    );
  }

  public getChildShare(
    p1MasterKeyShare: EcdsaParty1Share,
    xPos: number,
    yPos: number
  ): EcdsaParty1Share {
    const res = JSON.parse(
      bindings.p1_ecdsa_get_child_share(
